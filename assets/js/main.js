...
function scrollActive(){
    ...
    sections.forEach(current =>{
        ...
        if(scrollY > sectionTop && scrollY <= sectionTop + sectionHeight){
            document.querySelector('.nav__menu active a[href*=' + sectionId + ']').classList.add('active')
        }else{
            document.querySelector('.nav__menu active a[href*=' + sectionId + ']').classList.remove('active')
        }
    })
}
